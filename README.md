...
- **The end-of-call transcript-saving webhook has not been verified against a real inbound PSTN phone call**, only against Vapi's web-call widget (which, as noted above, carries no caller ID). The developer is based in Pakistan, and placing outbound test calls to the provisioned US number would incur international calling charges; no free/no-cost path to a real PSTN test call was available within the assessment's time window. The registration, lookup, and update flows were all fully verified over real phone calls placed earlier in development, so the tool-calling integration itself is confirmed working end-to-end over the phone — only the newer, separately-added end-of-call transcript webhook remains unverified specifically against a real (non-web) call. The endpoint code handles the expected Vapi payload shape and degrades safely (skips the save, logs the reason) if the phone number field is ever missing or in an unexpected format, but a live confirmation on a real call is a recommended first check during review.
...
- **Bonus features not attempted:** appointment scheduling and multi-language support were not built, in favor of hardening the core registration/update/duplicate-detection flow and adding call transcript storage and the dashboard within the time available.
